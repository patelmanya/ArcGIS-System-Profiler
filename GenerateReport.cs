...
namespace ArcGIS_System_Profiler
{
    public partial class GenerateReport : Form
    {
            ...
            try
            {
                ...
                if (globalVariables.ImageList.Count > 0)
                {
                    ...
                    if (tbl.Columns.Count == 1)
                    {
                        ...
                        tbl.Columns.Add();
                        tbl.Columns.Add();
                        tbl.Columns.PreferredWidth = 481;
                        tbl.Columns.Add();
                    }
                ...
                if (globalVariables.ImageList.Count > 1)
                {
                    ...
                    tbl.Cell(i, 1).Range.Text = "1.2.1";
                    tbl.Cell(i, 2).Range.Text = "ArcGIS Server Health Check";
                    tbl.Cell(i, 3).Range.Text = "Navigate to the ArcGIS Server Health Check URL as per the instructions here";
                    tbl.Cell(i, 4).Range.Text = "JSON Response \r\n { \"status\": \"success\"}";
                    tbl.Cell(i, 5).Range.Text = "Pass";
                    tbl.Rows.Add();
        ...
        private void btn_GenerateReport_Click_Word(object sender, EventArgs e)
        {
            try
            {
                btnOpenReport.Visible = false;
            ...
            try
            {
                ...
                if (globalVariables.generateReportList.Count > 0)
                {
                    foreach (string objArr in globalVariables.generateReportList)
                    {
                        ...
                        //NPOI.XWPF.UserModel.XWPFDocument docNEW = new NPOI.XWPF.UserModel.XWPFDocument();
                        
                        //docNEW
                        
                        //FileStream out1 = new FileStream(globalVariables.globalFilePath + "\\GeneratedServicesReportNEW_" + fileName + ".docx", FileMode.Create);
                        //docNEW.Write(out1);
                        //out1.Close();
        ...
        private void btnOpenReport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Document wordDoc = wordApp.Documents.Open(globalVariables.generatedFinalReportName);
            }
            catch (Exception ex)
            {
